...
# 0. MIDI over serial output with ESP32 and MicroPython

# Contents
0.  [MIDI over serial output with ESP32 and MicroPython](#0-midi-over-serial-output-with-esp32-and-micropython)
...
6.  [Turn off all notes that still might be on](#6-turn-off-all-notes-that-still-might-be-on)
7.  [Send all channel events to the MIDI output.](#7-send-all-channel-events-to-the-midi-output)
8.  [.play() will wait until the note is due.](#8-play-will-wait-until-the-note-is-due)
9.  [The receiving end](#9-the-receiving-end)
10.  [Some tinkering](#10-some-tinkering)
...
11.  [Other stuff](#11-other-stuff)
12.  [Troubleshooting](#12-troubleshooting)
13.  [Is this possible on a RP2040?](#13-is-this-possible-on-a-rp2040)
14.  [Links](#14-links)
...
# 6. Turn off all notes that still might be on
for channel in range(16):
    # 0xb0=Control Change, 120=All sound off 
    uart.write(bytes([0xb0+channel, 120])) 

# 7. Send all channel events to the MIDI output.
# 8. .play() will wait until the note is due.
...
# 13. Is this possible on a RP2040?
...
https://datasheets.raspberrypi.com/rp2040/rp2040-datasheet.pd
